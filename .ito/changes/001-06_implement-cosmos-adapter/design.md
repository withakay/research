...
- Deliver implement cosmos adapter as a focused package increment.
...
Map the core store protocol to Cosmos item operations. Use create-or-read idempotency for put, PATCH with If-Match for claims, query predicates for replay and cleanup, and startup capability validation for certified RPO=0 mode.
...
- Public behavior is specified in `specs/durable-outbox-cosmos-store/spec.md`.
...
Run the targeted pytest suite for this change and run `ito validate 001-06_implement-cosmos-adapter --strict`. Provider changes must pass the shared provider contract tests.
