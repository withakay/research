...
- Deliver implement single region blob store mvp as a focused package increment.
...
Represent each event as one deterministic JSON blob plus metadata/tags for discovery. Correctness comes from blob existence, metadata, ETag conditions, and list reconciliation; tags are an optimization, not the only source of truth.
...
- Public behavior is specified in `specs/durable-outbox-blob-store/spec.md`.
...
Run the targeted pytest suite for this change and run `ito validate 001-03_implement-blob-store-mvp --strict`. Provider changes must pass the shared provider contract tests.
