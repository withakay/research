...
- Deliver build core durable outbox library as a focused package increment.
...
Create a narrow core package first: models and protocols remain dependency-free, dispatcher logic depends only on the protocols, and provider certification tests exercise behavior through the public store interface. Cloud-specific behavior is represented through capability flags and test fixtures, not imported into core.
...
- Public behavior is specified in `specs/durable-outbox-core/spec.md`.
...
Run the targeted pytest suite for this change and run `ito validate 001-01_build-core-library --strict`. Provider changes must pass the shared provider contract tests.
