...
- Deliver implement sql adapter as a focused package increment.
...
Use a shared SQL record mapper and protocol implementation with capability-specific acceptance boundaries. Azure SQL sync mode performs commit plus sp_wait_for_database_copy_sync before success; Always On mode relies on configured synchronous commit semantics surfaced by validation.
...
- Public behavior is specified in `specs/durable-outbox-sql-store/spec.md`.
...
Run the targeted pytest suite for this change and run `ito validate 001-07_implement-sql-adapter --strict`. Provider changes must pass the shared provider contract tests.
