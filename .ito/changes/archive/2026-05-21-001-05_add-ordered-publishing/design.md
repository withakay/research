...
- Deliver add ordered publishing as a focused package increment.
...
Add ordering as an opt-in dispatch mode. Keep the core coordinator backend-agnostic, then implement Blob key locks using leases. The dispatcher asks the coordinator for eligible claims rather than embedding Blob-specific logic.
...
- Public behavior is specified in `specs/durable-outbox-ordering/spec.md`.
...
Run the targeted pytest suite for this change and run `ito validate 001-05_add-ordered-publishing --strict`. Provider changes must pass the shared provider contract tests.
