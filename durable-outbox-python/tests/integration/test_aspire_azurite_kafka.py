...
@pytest.mark.asyncio
async def test_azurite_blob_store_replays_pending_and_sent_to_local_file(
    tmp_path: Path,
) -> None:
    connection_string = _connection_string()
    if not connection_string:
    ...
    await client.ensure_container()
    store = BlobOutboxStore(client=client, environment="integration")
    dispatch_sink = FileSink(tmp_path / "dispatched.jsonl")
    replay_sink = FileSink(tmp_path / "replayed.jsonl")
    sent_event = _event(_unique_event_id("azurite-replay-sent"))
    pending_event = _event(_unique_event_id("azurite-replay-pending"))

    try:
        await store.put(sent_event)
        await store.put(pending_event)
        dispatch_summary = await OutboxDispatcher(store, dispatch_sink).run_once(
            limit=1
        )
        replay_summary = await FailoverReplayer(
            store,
            replay_sink,
            require_rpo_zero=False,
            replay_page_size=1,
        ).replay_once(
            failover_started_at=datetime.now(UTC) - timedelta(minutes=1),
            limit=10,
        )
    finally:
        await dispatch_sink.aclose()
        await replay_sink.aclose()
        await client.close()

    replayed_ids = {
        json.loads(line)["event_id"]
        for line in (tmp_path / "replayed.jsonl").read_text().splitlines()
    }
    assert dispatch_summary.sent == 1
    assert replay_summary.replayed == 2
    assert replayed_ids == {sent_event.event_id, pending_event.event_id}
