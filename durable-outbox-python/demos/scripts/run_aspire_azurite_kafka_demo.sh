...
test_resource="durable-outbox-integration-tests"
runtime="${ASPIRE_CONTAINER_RUNTIME:-podman}"
log_dir="${package_dir}/demos/.tmp"
mkdir -p "${log_dir}"
run_log="$(mktemp "${log_dir}/aspire-azurite-kafka.log.XXXXXX")"
...
python3 - "${aspire_dir}" "${apphost}" "${test_resource}" "${runtime}" "${run_log}" "${run_pid}" <<'PY'
import json
import os
import subprocess
import sys
...
while time.monotonic() < deadline:
    ...
    if test and test.get("state") == "Finished":
        ...
        print("demo=durable-outbox-python-aspire-azurite-kafka")
